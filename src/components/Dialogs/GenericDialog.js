...
const AddService = (props) => {
  ...
  useEffect(() => {
    if (props.tableRows && props.tableRows.length > 0) {
      // store the reg's that are already on the table to hide them from the reg dropdown.
        ...
        <div style={{ margin: '0px 50px' }}>
          <FormControl fullWidth>
            ...
            >
              {documents &&
              documents.filter(
                (vehicle) => !tableRegistrations.includes(vehicle.registration)
              ).length > 0 ? (
                documents
                  .filter(
                    (vehicle) =>
                      !tableRegistrations.includes(vehicle.registration)
                  ) // Filter out vehicles already in the registrations array
                  .map((vehicle, index) => (
